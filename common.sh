...
MC_MINOR_VERSION=$(echo "${MC_VERSION}" | cut -f1,2 -d'.')
export MC_MINOR_VERSION
...
RPM_MC_VERSION=$(echo $MC_VERSION | sed -r -r 's/(-)/\./g')
export RPM_MC_VERSION
RPM_PACKAGE_VERSION=$RPM_MC_VERSION-$RELEASE_VERSION
...
if [ "${EVENT_NAME}" == "pull_request" ]; then
  ...
  export DEBIAN_REPO_BASE_URL="https://${JFROG_USERNAME}:${JFROG_TOKEN}@repository.hazelcast.com/debian-test-local"
